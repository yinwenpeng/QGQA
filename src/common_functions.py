...
class Bd_LSTM_Batch_Tensor_Input_with_MaskandInit_Concate(object):
    # Bidirectional GRU Layer.
    def __init__(self, X, Mask,Init, hidden_dim, fwd_tparams, bwd_tparams):
        fwd = LSTM_Batch_Tensor_Input_with_MaskandInit(X, Mask, Init, hidden_dim, fwd_tparams)
        bwd = LSTM_Batch_Tensor_Input_with_MaskandInit(X[:,:,::-1], Mask[:,::-1], Init, hidden_dim, bwd_tparams)
...
class LSTM_Batch_Tensor_Input_with_MaskandInit(object):
#     def __init__(self, X, Mask, hidden_dim, U, W, b):
    def __init__(self, X, Mask, Init, hidden_size, tparams ):
        #X (batch, emb_size, senLen), Mask (batch, senLen)
        #Init:   (batch, hidden)
        state_below=X.dimshuffle(2,0,1)
        mask=Mask.T
...
class LSTM_Decoder_Train_with_Mask(object):
#     def __init__(self, X, Mask, hidden_dim, U, W, b):
    def __init__(self, X, encoder_res, vocab_embs, Mask, hidden_size, tparams ):
...
#         encoder_res = T.max(Encoder_Tensor_Rep, axis=2)  #(batch, hidden_size)
        X = T.concatenate([X, T.repeat(encoder_res.dimshuffle(0,1,'x'), X.shape[2], axis=2)], axis=1)  #(batch, hidden_size+emb_size, senLen)
        
        state_below=X.dimshuffle(2,0,1)    #(len, batch, hidden+emb)
        mask=Mask.T
        # state_below, (senLen, batch_size, emb_size)
        nsteps = state_below.shape[0] #sentence length, as LSTM or GRU needs to know how many words/slices to deal with sequentially
        if state_below.ndim == 3:
        ...
        rval, updates = theano.scan(_step,
                                    ...
                                    outputs_info=[T.alloc(numpy.asarray(0., dtype=theano.config.floatX),n_samples,dim_proj),
                                                  T.alloc(numpy.asarray(0., dtype=theano.config.floatX),n_samples,dim_proj)],
                                    n_steps=nsteps)
        self.output_tensor = rval[0].dimshuffle(1,2,0) # rval[0]: (senlen, batch, hidden_size), out_tensor: (batch, hidden, senlen)
        
        #now compare with hidden states with vocab embs
        '''
        occupy too much memory by repeat vocab emb
        '''
#         simi_tensor = T.batched_dot(T.repeat(vocab_embs.dimshuffle('x',0,1), self.output_tensor.shape[0], axis=0), self.output_tensor) #(batch, vocab_size, emb_size) * (batch, hidden_size, senLen) = (batch, vocab_size, senLen)
        simi_tensor = T.dot(self.output_tensor.dimshuffle(0,2,1), vocab_embs.T).dimshuffle(0,2,1)
        self.prob_matrix = T.nnet.softmax(simi_tensor.dimshuffle(0,2,1).reshape((X.shape[0]*X.shape[2], vocab_embs.shape[0])))#(batch*senlen, vocab_size)

class LSTM_Decoder_Train_with_Attention(object):
#     def __init__(self, X, Mask, hidden_dim, U, W, b):
    def __init__(self, X, Encoder_Tensor_Rep, Encoder_Mask, start_indices, end_indices, vocab_embs, Mask, emb_size, hidden_size, tparams, attention_para_dict1, attention_para_dict2):
        #X (batch, emb_size, senLen) is the ground truth target words
        #Encoder_Tensor_Rep   (batch, hidden_size, senLen) is the hidden states from source side
        #hidden_size: the hidden size for the target side, we set it equal to emb size, so that compare with each vocab word emb
        #Mask: mask for target sentences
        #vocab_embs:  matrix (vocab_size, emb_size)
        #start_index, end_index: for answer boudansy, a vector with dimension of batch
        ...
        def _step(m_, x_, h_, c_): #mask, x_ current word embedding, h_ and c_ are hidden states in preceding step of two hidden layers
            #first attention over encoder hidden states
            #h_ (batch, hidden_size), Encoder_Tensor_Rep  (batch, hidden_size, senlen)
            attention_model=Bd_LSTM_Batch_Tensor_Input_with_MaskandInit_Concate(Encoder_Tensor_Rep, Encoder_Mask, h_, hidden_size, attention_para_dict1, attention_para_dict2)
            att_reps_tensors = attention_model.output_tensor
            batch_ids=T.arange(Encoder_Tensor_Rep.shape[0])
            ans_heads=att_reps_tensors[batch_ids,:,start_indices]
            ans_tails=att_reps_tensors[batch_ids,:,end_indices]
            
            l_context_heads = att_reps_tensors[:,:,0]
            l_context_tails = att_reps_tensors[batch_ids,:,start_indices-1]
            
            r_context_heads = att_reps_tensors[batch_ids,:,end_indices+1]
            r_context_tails = att_reps_tensors[:,:,-1]
            
            att_reps = T.concatenate([l_context_heads,l_context_tails, ans_heads, ans_tails, r_context_heads, r_context_tails], axis=1) #(batch, 6*2hidden_size)
            
            att_x_ = T.concatenate([x_, att_reps], axis=1) #(batch, 6*2hidden+emb_size)
            
            
            
            preact = T.dot(h_, tparams['U'])
            proj_att_x_ = T.dot(att_x_, tparams['W']) + tparams['b']  #(batch, emb_size)
            preact += proj_att_x_
            
            i = T.nnet.sigmoid(_slice(preact, 0, emb_size))
            f = T.nnet.sigmoid(_slice(preact, 1, emb_size))
            o = T.nnet.sigmoid(_slice(preact, 2, emb_size))
            c = T.tanh(_slice(preact, 3, emb_size))
...
#         assert mask is not None
        ...
        def _slice(_x, n, dim):
            #split (batch. n*hidden) into n (batch, hidden)
            if _x.ndim == 3:
                return _x[:, :, n * dim:(n + 1) * dim]
            return _x[:, n * dim:(n + 1) * dim]
    
        def _step(ids, x_, h_, c_): #mask, x_ current word embedding, h_ and c_ are hidden states in preceding step of two hidden layers
            #x_, h_, c_:  all are updated, pred_emb, h_, c_
            attention_model=Bd_LSTM_Batch_Tensor_Input_with_MaskandInit_Concate(Encoder_Tensor_Rep, Encoder_Mask, h_, hidden_size, attention_para_dict1, attention_para_dict2)
            att_reps_tensors = attention_model.output_tensor
            batch_ids=T.arange(Encoder_Tensor_Rep.shape[0])
            ans_heads=att_reps_tensors[batch_ids,:,start_indices]
            ...
            x_ = T.concatenate([x_, att_reps], axis=1) #(batch, emb_size+encoding_hidden_size)
            x_ = T.dot(x_, tparams['W']) + tparams['b']  #(batch, input_size) *(input_size, 4*hidden_size) = (batch, 4*hidden)
            preact = T.dot(h_, tparams['U'])  #(batch, hidden) *(hidden, 4*hidden) = (batch, 4*hidden)
            preact += x_
            
            i = T.nnet.sigmoid(_slice(preact, 0, emb__size))
            f = T.nnet.sigmoid(_slice(preact, 1, emb__size))
            o = T.nnet.sigmoid(_slice(preact, 2, emb__size))
            c = T.tanh(_slice(preact, 3, emb__size))
...
#         state_below = (T.dot(state_below, tparams['W']) + tparams['b'])#(senlen, batch, emb_size) * (emb_size, 4*hidden_size) = (senlen, batch, 4*hidden_size)
    
        dim_proj = emb__size
        rval, updates = theano.scan(_step,
                                    outputs_info=[T.alloc(numpy.asarray(0, dtype='int64'),n_samples),
                                                  T.alloc(numpy.asarray(0., dtype=theano.config.floatX),n_samples,dim_proj),
                                                  T.alloc(numpy.asarray(0., dtype=theano.config.floatX),n_samples,dim_proj),
                                                  T.alloc(numpy.asarray(0., dtype=theano.config.floatX),n_samples,dim_proj)],
                                    n_steps=nsteps)
        self.output_id_matrix = rval[0].T # rval[0]: (senlen, batch), output_id_matrix: (batch, senlen)

class LSTM_Decoder_Test_with_Mask(object):
#     def __init__(self, X, Mask, hidden_dim, U, W, b):
    def __init__(self, nsteps, encoder_res, vocab_embs, hidden_size, tparams ):
        #X (batch, emb_size, senLen) is the ground truth target words
        #Encoder_Tensor_Rep   (batch, hidden_size, senLen) is the hidden states from source side
        #hidden_size: the hidden size for the target side, we set it equal to emb size, so that compare with each vocab word emb
        #Mask: mask for target sentences
        ...
        if encoder_res.ndim == 2:
            n_samples = encoder_res.shape[0] #batch_size
        else:
            n_samples = 1
...
#         assert mask is not None
        ...
        def _step(ids, x_, h_, c_): #mask, x_ current word embedding, h_ and c_ are hidden states in preceding step of two hidden layers
            #x_, h_, c_:  all are updated, pred_emb, h_, c_
            x_ = T.concatenate([x_, encoder_res], axis=1) #(batch, emb_size+encoding_hidden_size)
            x_ = T.dot(x_, tparams['W']) + tparams['b']  #(batch, input_size) *(input_size, 4*hidden_size) = (batch, 4*hidden)
            preact = T.dot(h_, tparams['U'])  #(batch, hidden) *(hidden, 4*hidden) = (batch, 4*hidden)
            preact += x_
            i = T.nnet.sigmoid(_slice(preact, 0, hidden_size))
