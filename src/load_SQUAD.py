...
#     with open(os.path.join(os.path.dirname(os.path.realpath(__file__)), "data/glove/glove.6B." + str(dim) + "d.txt")) as f:
    f=open('/mounts/data/proj/wenpeng/Dataset/glove.6B.50d.txt', 'r')  #glove.6B.100d.txt, glove.840B.300d.txt
    for line in f:
        l = line.split()
        word2vec[l[0]] = map(float, l[1:])
...
def load_QGQA(word2id, max_para_len, max_q_len, top_n_Qwords, train):
    if train:
        readfile=codecs.open(path+'train-QGQA.txt', 'r', 'utf-8')
    else:
        readfile=codecs.open(path+'dev-QGQA.txt', 'r', 'utf-8')
    paras=[]
    para_masks=[]
    questions=[]
    question_masks=[]
    ...
    for line in readfile:
        ...
        if len(parts)!=4:
            ...
            ans=parts[1].strip().split()
            right = parts[2].strip().split()
            question = parts[3].strip().split()
            
            para = left+ans+right
            if len(para) > max_para_len or len(right)==0 or len(left)==0:
                too_long+=1
                continue
            
            para_idlist, para_masklist=transfer_wordlist_2_idlist_with_maxlen(para, word2id, max_para_len)     
            if max_para_len > len(para):
                start = len(left)+(max_para_len-len(para))
                end = len(left+ans)-1+(max_para_len-len(para))
            else:
                start = len(left)
                end = len(left+ans)-1
...
#             top_q_wordid, _ = transfer_wordlist_2_idlist_with_maxlen(question[:top_n_Qwords], word2id, top_n_Qwords) 
            ...
            para_masks.append(para_masklist)
            questions.append(q_idlist)
            question_masks.append(q_masklist)
            ans_starts.append(start)
            ans_ends.append(end)
    readfile.close()
    print 'data load over, too long:', too_long, ', remain train size:', len(paras)
    return             paras,para_masks,questions,question_masks,ans_starts,ans_ends, top_n_Q_wordids, word2id
    
def  reformat_SQUADTrain_into_QGQA():

    
    writefile=codecs.open(path+'train-QGQA.txt', 'w', 'utf-8')
...
#                 answer_right_wordlist=truncate_by_punct(tokenize(answer_right), False)
                ...
                writefile.write(' '.join(truncate_by_punct(answer_left_wordlist, True))+'\t;\t'+' '.join(answer_q_wordlist)+'\t;\t'+' '.join(truncate_by_punct(answer_right_wordlist, False))+'\t;\t'+' '.join(question_wordlist)+'\n')
...
def  reformat_SQUADDev_into_QGQA():
#     Dev  max_para_len:, 629 max_q_len: 33
    writefile=codecs.open(path+'dev-QGQA.txt', 'w', 'utf-8')
    with open(path+'dev-v1.1.json') as data_file:
        data = json.load(data_file)
    doc_size=len(data['data'])
...
#                 print 'q_id:', q_id
                ...
                for ans in range(answer_no):
                    answer_q=data['data'][i]['paragraphs'][j]['qas'][q]['answers'][ans]['text']
                    if answer_q.strip() == old_ans:
                        continue
                    else:
                        old_ans = answer_q.strip()
                        answer_q_wordlist= tokenize(answer_q.strip())
...
def refine_decoder_predictions(predlist, grounds, masklist):
    predlist.append(0)
    preds=[]
    for id, value in enumerate(predlist):
        if value!=0: 
            if id < len(predlist)-1:
                if value !=predlist[id+1]:
                    preds.append(value)
            else:
                preds.append(value)
#         else:
#             break
    ...
    while masklist[i]==0:
        i-=1
    if i<0:
        print 'fuck, all ground truth ids are zero'
        exit(0)
    return preds, grounds[:i+1]
...
#     store_SQUAD_dev()
#     filt_data()
#     reformat_SQUADTrain_into_QGQA()
#     reformat_SQUADDev_into_QGQA()

    score = nltk.translate.bleu_score.sentence_bleu([['haha', 'ni', 'mei']], ['we','are', 'friends','haha'])
    print score
